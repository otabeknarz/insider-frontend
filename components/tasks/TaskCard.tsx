...
export function TaskCard({ task, onEdit, onDelete, onDragStart }: TaskCardProps) {
  ...
  const priorityLabels: Record<number, string> = {
    [TaskPriority.MEDIUM]: t("tasks.priorityMedium") === "tasks.priorityMedium" ? "Medium" : t("tasks.priorityMedium"),
    [TaskPriority.HIGH]: t("tasks.priorityHigh") === "tasks.priorityHigh" ? "High" : t("tasks.priorityHigh"),
  };
  
  const statusLabels: Record<number, string> = {
    [TaskStatus.ASSIGNED]: t("tasks.assigned") === "tasks.assigned" ? "Assigned" : t("tasks.assigned"),
    [TaskStatus.RECEIVED]: t("tasks.received") === "tasks.received" ? "Received" : t("tasks.received"),
    [TaskStatus.IN_PROCESS]: t("tasks.inProcess") === "tasks.inProcess" ? "In Process" : t("tasks.inProcess"),
    [TaskStatus.COMPLETED]: t("tasks.completed") === "tasks.completed" ? "Completed" : t("tasks.completed"),
  };

  // Color schemes for priority and status
  const priorityColors: Record<number, string> = {
    [TaskPriority.MEDIUM]: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300",
    [TaskPriority.HIGH]: "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300",
  };

  const statusColors: Record<number, string> = {
    [TaskStatus.ASSIGNED]: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300",
    [TaskStatus.RECEIVED]: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300",
    [TaskStatus.IN_PROCESS]: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300",
    [TaskStatus.COMPLETED]: "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-300",
  };
  
  const assignedToText = t("tasks.assignedTo") === "tasks.assignedTo" ? "Assigned to" : t("tasks.assignedTo");
  const deadlineText = t("tasks.deadline") === "tasks.deadline" ? "Deadline" : t("tasks.deadline");
          ...
          {task.assigned_users && task.assigned_users.length > 0 && (
            <div className="flex items-center">
              ...
              <div className="flex -space-x-1">
                ...
                {task.assigned_users.filter(user => user && typeof user === 'object' && user.id).length > 3 && (
