...
import { ChevronLeft, Edit, Plus, Trash2, Users, UserPlus, X, ClipboardList, Home, Loader2, Save, Search, Check, User, ExternalLink, ArrowRight } from "lucide-react";
...
import { Task as BackendTask, TaskStatusBackend, TaskPriorityBackend } from "@/lib/types";
...
export default function TeamDetailPage() {
  ...
  const fetchAvailableUsers = async (search = "") => {
    ...
    try {
      ...
      const filteredUsers = users.filter(user => !existingMemberIds.has(user.id));
  ...
  useEffect(() => {
    const fetchTeamTasks = async () => {
      ...
      try {
        ...
        const response = await ApiService.request<{ results?: BackendTask[], data?: BackendTask[] } | BackendTask[]>({
          method: 'get',
      ...
      try {
        ...
        if (taskToEdit) {
          ...
          setFormData({
            ...
            assigned_users: taskToEdit.assigned_users?.map((user: any) => 
  ...
  const handleEditTask = (task: BackendTask) => {
    ...
    setFormData({
      ...
      team: (typeof task.team === 'object' && task.team !== null ? (task.team as any).id : task.team) || team?.id || null,
      assigned_users: task.assigned_users?.map((user: any) => 
        typeof user === "object" ? user.id : user
      ) || [],
  ...
  const handlePriorityChange = async (task: BackendTask, newPriority: number) => {
    ...
    <div className="container mx-auto py-8">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button asChild>
            <Link href={`/teams/${id?.toString()}/tasks`} className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isAdminOrOwner() && (
                              <Button 
                                ...
                                onClick={() => handleOpenRemoveUserDialog(member.id)}
                              >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isAdminOrOwner() && (
                              <Button 
                                ...
                                size="icon" 
                                className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10"
                                onClick={() => handleOpenRemoveUserDialog(admin.id)}
                              >
                                <Trash2 className="h-4 w-4" />
                              </Button>
                            )}
          ...
          <form onSubmit={handleTeamSubmit} className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={handleTeamFormChange}
                placeholder={t("teams.enterTeamDescription") || "Enter team description"}
                rows={4}
              />
            </div>
            
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setIsEditTeamDialogOpen(false)} disabled={savingTeam}>
      ...
      <Dialog open={isAddMemberDialogOpen} onOpenChange={setIsAddMemberDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("teams.addMembersDescription") || "Select users to add as members to this team."}
            </DialogDescription>
          </DialogHeader>
            ...
            {selectedUserIds.length > 0 && (
              <div className="flex flex-wrap gap-1 p-2 border rounded-md bg-muted/50">
                <p className="w-full text-xs text-muted-foreground mb-1">
                  {t("teams.selectedUsers", { count: selectedUserIds.length }) || 
                ...
                {selectedUserIds.map(id => {
                  ...
                  if (!user) return null;
                  return (
                    <div key={id} className="flex items-center gap-1 text-xs bg-background rounded-full pl-2 pr-1 py-1">
                  ...
                  <div className="p-1">
                    {availableUsers.map((user) => (
                      <div
                        ...
                        className={`flex items-center justify-between p-2 rounded-md cursor-pointer hover:bg-muted ${selectedUserIds.includes(user.id) ? 'bg-muted' : ''}`}
                      >
                        <div className="flex items-center gap-2">
                          <Avatar className="h-8 w-8">
                            <AvatarFallback>
      ...
      <Dialog open={isAddAdminDialogOpen} onOpenChange={setIsAddAdminDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("teams.addAdminsDescription") || "Select users to add as administrators to this team."}
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={isRemoveUserDialogOpen} onOpenChange={setIsRemoveUserDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("teams.removeUserConfirmation") || "Are you sure you want to remove this user from the team?"}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
